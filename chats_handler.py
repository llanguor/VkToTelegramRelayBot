...
def get_channel_destinations(source_id):
    for channel in data.values():
        if str(channel.get("source")) == str(source_id):
            return channel.get("destinations", [])
    return []

def change_subscription(channel_id, botname, chat_id):
    chat_id = str(chat_id)
    ...
    with lock:
        ...
        if chat_id not in data[channel_id]["destinations"][botname]:
            data[channel_id]["destinations"][botname].append(chat_id)
            save_data(FILE, data)
            logger.info(f"User with chat id {chat_id} for bot {botname} subscribed to {channel_id}")
            return True
        else:
            data[channel_id]["destinations"][botname].remove(chat_id)
            save_data(FILE, data)
            logger.info(f"User with chat id {chat_id} for bot {botname} unsubscribed from {channel_id}")
            return False
